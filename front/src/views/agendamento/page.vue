...
const fetchData = async () => {
  ...
  try {
    ...
    // 1. Identifica minhas instituições
    ...
    // 2. Filtra Salas (Só mostra salas das minhas instituições)
    const myRoomIds = new Set<number>()
    if (resRooms.data.success) {
      const filteredRooms = resRooms.data.data.filter((room: any) => myInstIds.has(room.fk_instituicao_id))
      
      classrooms.value = filteredRooms.map((sala: any) => {
        myRoomIds.add(sala.id) // Guarda ID para filtrar agendamentos depois
        return {
          id: sala.id,
          name: sala.nome,
          capacity: 30, 
    ...
    // 3. Filtra Agendamentos (Só mostra de salas que eu vejo)
    if (resBookings.data.success) {
      const allBookings = resBookings.data.data
      
      recentReservations.value = allBookings
        .filter((b: any) => myRoomIds.has(b.fk_salas_id)) // FILTRO DE SEGURANÇA
        .slice(0, 10)
        .map((b: any) => {
          let startTime = b.horario_inicio
          if (!startTime && b.horarios?.length) startTime = b.horarios[0].inicio

          let roomName = 'Sala ID ' + b.fk_salas_id
          if (b.sala?.nome) roomName = b.sala.nome
          else {
...
// ... Lógica de Cancelamento (igual ao anterior) ...
...
const confirmCancel = async () => {
  ...
  try {
    await api.delete(`/agendamento/deletar/${bookingToCancel.value}`)
    ...
    <div class="flex-1 flex flex-col overflow-hidden">
          ...
          <div v-else>
            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
              <div v-if="classrooms.length === 0" class="col-span-full text-center py-8 text-slate-500 border border-dashed rounded-lg">
                Nenhuma sala encontrada nas suas instituições.
              </div>
              <div v-for="room in classrooms" :key="room.id" class="bg-white rounded-lg border border-slate-200 p-0 flex flex-col hover:border-rose-600 hover:shadow-md transition-all duration-200 group">
                <div class="p-5 flex-1">
                  ...
                  <div class="space-y-3">
                    <div class="flex items-center gap-2 text-sm text-slate-500"><Users class="w-4 h-4 text-slate-400 flex-shrink-0" /><span>Capacidade: {{ room.capacity }}</span></div>
                    <div class="flex items-center gap-2 text-sm text-slate-500"><Clock class="w-4 h-4 text-slate-400 flex-shrink-0" /><span>Ver grade horária</span></div>
                  </div>
                </div>
                  ...
                  <div class="space-y-3">
                    <div v-for="res in recentReservations" :key="res.id" class="flex flex-col sm:flex-row sm:items-center justify-between p-4 rounded-md border border-slate-100 hover:bg-slate-50 transition-colors gap-3 group">
                      ...
                      <div class="flex items-center gap-3">
                        ...
                        <button @click="openCancelModal(res.id)" class="p-2 text-slate-300 hover:text-red-600 hover:bg-red-50 rounded-md transition-all opacity-100 sm:opacity-0 sm:group-hover:opacity-100" title="Cancelar"><Trash2 class="w-4 h-4" /></button>
                      </div>
                    </div>
                  </div>
                </div>
      ...
      <Modal :is-open="showCancelModal" title="Cancelar Reserva" @close="showCancelModal = false">
        <div class="text-center space-y-4">
          ...
          <p class="text-sm text-slate-500 leading-relaxed">Você está prestes a cancelar este agendamento.</p>
          <div class="flex gap-3 pt-4">
            <button @click="showCancelModal = false" class="flex-1 h-11 rounded-md border border-slate-300 font-bold text-slate-700 hover:bg-slate-50">Voltar</button>
            <button @click="confirmCancel" :disabled="isCanceling" class="flex-1 h-11 rounded-md bg-red-600 text-white font-bold hover:bg-red-700 flex items-center justify-center gap-2 shadow-sm"><Loader2 v-if="isCanceling" class="w-4 h-4 animate-spin" /><span v-else>Sim, Cancelar</span></button>
          </div>
        </div>
      </Modal>
    </div>
